...
server <- function(input, output, session) {
  ...
  InitValues <- reactive({
    ...
    Data.TotalQty<- length(vectorTime)
    if(length(sliderArrage) == 0) {
      print("shi0000")
    }
    else
    {
      StartIdx<-which(vectorTime==sliderArrage[1])
      EndIdx<-which(vectorTime==sliderArrage[2])
      if(StartIdx == 1& EndIdx==Data.TotalQty)
      {
        print("shi11111")
      }
      else
    ...
    data.frame(
      CH2_2 = c("速度",
               "周期",
               "异常周期"),
      Value = as.character(c(1,#nrow(csvdata()),#1,#max(csvdata$Time.s.),
                             2,
                             3),
  ...
  output$slider <- renderUI({
    ...
    sliderInput("slider", "请拖动端点选择图形区间: ", min=sliderMin,max=sliderMax,value = c(sliderMin,sliderMax))
  })
  ...
  output$checkboxGroupInput <- renderUI({
    df<-csvdata()
    vectorCHs<- colnames(df)
    checkboxGroupInput("icons", "请勾选通道:",choices=vectorCHs[-1],selected=vectorCHs[-1]
    )
  })
